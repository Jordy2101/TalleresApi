...
namespace Talleres.Controllers
{
    ...
    [ApiController]
    public class ListReciboController : Controller
    {

        readonly IListReciboEngine service;
        public ListReciboController(IListReciboEngine _service)
        {
            service = _service;
        }

        [HttpGet]
        public ActionResult GetPaged([FromQuery] ListVReciboFilter filter)
        {
            try
            {
                var data = service.GetPaged(filter);
                var result = PagedList<VRecibo>.Create((IQueryable<VRecibo>)data.AsQueryable(), filter.PageNumber, filter.PageSize);
